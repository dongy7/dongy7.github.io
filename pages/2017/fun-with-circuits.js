...
export default () =>
  <Post>
    ...
    <P>
      ...
      in my examples because it offers a very nice type system to work with. If
      you know JavaScript, then you’ll be able to follow along with a basic
      understanding of the type system offered by TypeScript.
    </P>

    <P>
      I’ve been solving various problems on the{' '}
      <Link to={'http://adventofcode.com/'}>Advent of Code</Link> and I came
      across a pretty interesting problem on{' '}
    ...
    <P>
      ...
      break the problem down into its essential part, i.e. how do we get the
      value of a given wire? If you take a look at the test input, then you’ll
      notice that the wires on the right hand side of the <Code>-></Code> are
      all unique.
    </P>
    ...
    <P>
      ...
      why a wire can’t have more than one output.
    </P>
...
x AND y -> z`}
    ...
    <P>
      ...
      of bitwise ANDing wire <Code>x</Code> and wire <Code>y</Code>’s values. So
      we just need to get the value of wire <Code>x</Code> and wire{' '}
      <Code>y</Code> and we have found wire <Code>z</Code>’s value.
    </P>

    <P>
      However, there is a small problem. What if we don’t know the values of{' '}
      <Code>x</Code> or <Code>y</Code> when we are trying to compute{' '}
      <Code>z</Code>? Well, we know that some of these rules can be evaluated
      without any knowledge of the state of other wires, such as the rule{' '}
      <Code>123 -> x</Code>. So, after all of these rules have been evaluated,
      we can evaluate the rules that depend on the wires that have now been
      initialized.
    </P>
...
interface Circuit {
  [id: string]: Wire;
    ...
    <P>Now we can build the function that assembles the board:</P>
...
const assemble = (instructions: Instruction[]): Circuit => {
  ...
  for (const inst of instructions) {
    ...
    <P>
      ...
      all of the wires are polling from the same board. For the <Code>AND</Code>{' '}
      case, we simply poll operand <Code>A</Code>
      and operand <Code>B</Code>. If either of these operands is{' '}
      <Code>undefined</Code>, we simply return <Code>undefined</Code>. Otherwise
